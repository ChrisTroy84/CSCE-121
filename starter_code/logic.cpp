...
char** loadLevel(const string& fileName, int& maxRow, int& maxCol, Player& player) {
    ...
    if (ifs.fail() || maxRow < 1 || maxCol < 1 || maxRow > (INT32_MAX / maxCol) || player.row < 0 || player.row >= maxRow ||
    ...
    for (i = 0; i < maxRow; ++i) {
        for (j = 0; j < maxCol; ++j) {
            ...
            if (curr_tile == TILE_DOOR) {
                dungeon[i][j] = curr_tile;
                exit = true;
            } else if (curr_tile == TILE_EXIT) {
                dungeon[i][j] = curr_tile;
                exit = true;
            } else if (curr_tile == TILE_OPEN) {
                dungeon[i][j] = curr_tile;
            } else if (curr_tile == TILE_PLAYER) {
                dungeon[i][j] = curr_tile;
            } else if (curr_tile == TILE_TREASURE) {
                dungeon[i][j] = curr_tile;
            } else if (curr_tile == TILE_AMULET) {
                dungeon[i][j] = curr_tile;
            } else if (curr_tile == TILE_MONSTER) {
                dungeon[i][j] = curr_tile;
            } else if (curr_tile == TILE_PILLAR) {
                dungeon[i][j] = curr_tile;
            } else {
                deleteMap(dungeon, maxRow);
                return nullptr;
...
char** createMap(int maxRow, int maxCol) {
    ...
    char** curr_tile = new char*[maxRow];
    char* curr;
    int i = 0;
    int j = 0;
...
char** resizeMap(char** map, int& maxRow, int& maxCol) {
    ...
    map[curr_row][curr_col] = TILE_OPEN;
